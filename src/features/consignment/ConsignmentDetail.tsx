...
const ConsignmentDetail: React.FC = () => {
  ...
  // Gọi tất cả các Hooks trước bất kỳ lệnh return nào
  const { data, isLoading: isBuildingLoading } = useQuery({
  ...
  // Xử lý logic sau khi đã gọi tất cả Hooks
  useEffect(() => {
    if (building) {
      form.setFieldsValue({
        ...building,
                    ...
                    >
                      {buildingUnit.rentalPricing?.length ? (
                        <div style={{ marginBottom: "8px" }}>
                          {buildingUnit.rentalPricing[
                            buildingUnit.rentalPricing.length - 1
                          ]?.price ? (
                            <>
                              {formatCurrency(
                                buildingUnit.rentalPricing[
        ...
        {currentTab === "approval" && (
          <>
            ...
            <div className="flex gap-8">
              ...
              <Access
                permission={PERMISSIONS[Module.BUILDINGS].UPDATE_BUILDING}
                hideChildren={true}
              >
